...
class SignedMapper extends TransactionMapper
{
	protected string $className = Signed::class;
	...
	/**
	 * Count Signatures Signeds stored in DB according to given parameter
	 *
	 * @param	string		$id
	 * @return	array		CDEM
	 * @throws	Throwable
	...
	private function _commonCount(
		...
		try {
			...
			// Add more filters
			$this->whereGlobalStatusSigned($queryBuilder);
	...
	/**
	 * Count Signatures Signeds stored in DB for a specific Applicant
	 *
	...
	/**
	 * Count the Transactions Signeds according to the TransactionID
	...
	public function findByApplicant(
		...
		try {
			...
			$rows = $this->findEntities($queryBuilder);

			$cdem->setOk(
				data: [
					CstDatabase::TRANSACTIONS => $rows,
				]
			);
		} catch (\Throwable $th) {
			$cdem->setError($th, $this->logRCDevs);
